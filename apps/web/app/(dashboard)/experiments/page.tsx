    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-lg">
            Detailed logs of numerical method computations and benchmarking.
          </p>
        </div>
                ...
                {isLoading
                  ? [...Array(6)].map((_, i) => (
                      <tr key={i} className="animate-pulse">
                        <td
                          colSpan={6}
                          className="px-8 py-6 h-16 bg-slate-900/10"
                        />
                      </tr>
                    ))
                  : (data?.items || []).map((exp) => (
                      ...
                      >
                        ...
                        <td className="px-8 py-6 font-mono font-bold text-white text-lg">
                          {formatCurrency(exp.computed_price)}
                        </td>
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-2">
                            <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                            <span className="text-slate-400 text-xs font-mono">
                              {(exp.exec_seconds * 1000).toFixed(3)}ms
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-8 py-6">
                          <div className="flex gap-2">
                            ...
                            >
                              S:{exp.option_parameters.underlying_price}
                            </Badge>
                            <Badge
                              variant="slate"
                              className="text-[10px] font-mono tracking-tighter"
                            >
                              K:{exp.option_parameters.strike_price}
                            </Badge>
                          </div>
                        </td>
                      </tr>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500 font-medium">
          Showing <span className="text-slate-300">{(page - 1) * 12 + 1}</span>{" "}
          to{" "}
          <span className="text-slate-300">
            {Math.min(page * 12, data?.total || 0)}
          </span>{" "}
          of <span className="text-slate-300">{data?.total || 0}</span> results
        </p>
        <div className="flex gap-2">
          <Button
            variant="outline"
