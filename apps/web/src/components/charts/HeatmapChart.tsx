  ...
  return (
    <div className="w-full overflow-x-auto pb-4">
      <div className="inline-grid gap-1" style={{ gridTemplateColumns: `auto repeat(${xLabels.length}, 1fr)` }}>
        <div /> {/* Top-left spacer */}
        {xLabels.map(label => (
          <div key={label} className="text-[10px] font-bold text-slate-500 text-center uppercase tracking-tighter py-2">
            {label}
          </div>
        ))}
        ...
        {yLabels.map(yLabel => (
          <>
            <div key={yLabel} className="text-[10px] font-bold text-slate-500 pr-4 flex items-center justify-end uppercase whitespace-nowrap">
              {yLabel}
            </div>
            {xLabels.map(xLabel => {
