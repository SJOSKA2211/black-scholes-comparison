...
class TestNotifications:
    ...
    async def test_notify_user_hierarchy_success(
        self, mock_push, mock_email, mock_redis, mock_insert
    ) -> None:
        mock_r = MagicMock()
    ...
    @pytest.mark.asyncio
    @patch("src.notifications.hierarchy.insert_notification")
    async def test_notify_user_hierarchy_failure(self, mock_insert) -> None:
    ...
    async def test_email_service_success(self, mock_resend_send, mock_settings) -> None:
        mock_settings.return_value.resend_api_key = "key-123"
        mock_resend_send.return_value = MagicMock(id="email-123")
        res = await send_email("test@example.com", "Subject", "Content")
        assert res is True

    @pytest.mark.asyncio
    @patch("src.notifications.email.get_settings")
    async def test_email_service_no_key(self, mock_settings) -> None:
    ...
    @patch("src.notifications.email.get_settings")
    @patch("resend.Emails.send")
    async def test_email_service_failure(self, mock_resend_send, mock_settings) -> None:
        mock_settings.return_value.resend_api_key = "key-123"
    ...
    async def test_push_service_success(self, mock_repo) -> None:
        mock_repo.get_push_subscriptions = AsyncMock(
            return_value=[{"id": 1, "subscription_info": {"endpoint": "..."}}]
        )
    ...
    @pytest.mark.asyncio
    @patch("src.notifications.push.repository")
    async def test_push_service_partial_failure(self, mock_repo) -> None:
    ...
    @pytest.mark.asyncio
    @patch("src.notifications.push.repository")
    async def test_push_service_no_subs(self, mock_repo) -> None:
    ...
    @pytest.mark.asyncio
    @patch("src.notifications.push.repository")
    async def test_push_service_failure(self, mock_repo) -> None:
