...
class TestAuthDeps:
    ...
    async def test_verify_ws_token_success(self, mock_get_supabase):
        mock_ws = AsyncMock()
        mock_user = MagicMock()
        mock_user.id = "u1"
    ...
    @pytest.mark.asyncio
    async def test_verify_ws_token_missing(self):
        mock_ws = AsyncMock()
...
class TestNotifications:
    ...
    async def test_notify_user(self, mock_get_redis, mock_insert_notif):
