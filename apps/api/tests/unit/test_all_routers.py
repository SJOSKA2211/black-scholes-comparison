...
class TestAllRouters:
    def test_market_data_router(self, mock_auth):
        with patch("src.routers.market_data.get_market_data") as mock_get:
            mock_get.return_value = [{"id": "1", "price": 100.0}]
            response = client.get("/api/v1/market-data/?source=spy", headers={"Authorization": "Bearer token"})
            assert response.status_code == 200
            assert response.json()[0]["price"] == 100.0

    def test_scrapers_router_trigger(self, mock_auth):
        with patch("src.routers.scrapers.publish_scrape_task") as mock_pub:
            response = client.post("/api/v1/scrapers/trigger?market=spy", headers={"Authorization": "Bearer token"})
            assert response.status_code == 200
            mock_pub.assert_called_once()

    def test_scrapers_router_runs(self, mock_auth):
        with patch("src.routers.scrapers.get_scrape_runs") as mock_get:
            mock_get.return_value = [{"id": "run-1"}]
            response = client.get("/api/v1/scrapers/runs", headers={"Authorization": "Bearer token"})
            assert response.status_code == 200
            assert response.json()[0]["id"] == "run-1"

    def test_experiments_router_run(self, mock_auth):
        with patch("src.routers.experiments.publish_experiment_task") as mock_pub:
            response = client.post("/api/v1/experiments/run", json={"test": "data"}, headers={"Authorization": "Bearer token"})
            assert response.status_code == 200
            mock_pub.assert_called_once()
    ...
    def test_notifications_router(self, mock_auth):
        ...
        with patch("src.routers.notifications.mark_notification_read") as mock_patch:
            response = client.patch("/api/v1/notifications/notif-1/read", headers={"Authorization": "Bearer token"})
            assert response.status_code == 200
