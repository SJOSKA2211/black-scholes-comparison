...
class TestMissingCoverage:
    ...
    def test_analytical_iv_failure(self):
    ...
    def test_fdm_boundary_delta(self):
        params_low = OptionParams(
            underlying_price=0.0001, strike_price=100, maturity_years=1,
            volatility=0.2, risk_free_rate=0.05, option_type="call"
        )
        ...
        params_high = OptionParams(
            underlying_price=500.0, strike_price=100, maturity_years=1,
            volatility=0.2, risk_free_rate=0.05, option_type="call"
        )
    ...
    def test_explicit_fdm_cfl_bumping(self):
        params = OptionParams(
            underlying_price=100, strike_price=100, maturity_years=1,
            volatility=0.22, risk_free_rate=0.05, option_type="call"
        )
        method = ExplicitFDM(num_time_steps=10, num_price_steps=10)
        method.price(params)

    def test_mc_replications_branch(self):
        params = OptionParams(
            underlying_price=100, strike_price=100, maturity_years=0.0001,
            volatility=0.2, risk_free_rate=0.05, option_type="call"
        )
    ...
    def test_tree_steps_branch(self):
        params = OptionParams(
            underlying_price=100, strike_price=100, maturity_years=0.0001,
            volatility=0.2, risk_free_rate=0.05, option_type="call"
        )
    ...
    def test_richardson_none_greeks(self):
        params = OptionParams(
            underlying_price=100, strike_price=100, maturity_years=1,
            volatility=0.2, risk_free_rate=0.05, option_type="call"
        )
    ...
    async def test_pricing_router_compare_success_with_analytical(self):
    ...
    async def test_pricing_router_compare_exception(self):
        params = OptionParams(
            underlying_price=100, strike_price=100, maturity_years=1,
            volatility=0.2, risk_free_rate=0.05, option_type="call"
        )
        mock_user = {"id": "test-user"}
        with patch("src.routers.pricing.get_method_instance", side_effect=Exception("Compare failed")):
            with pytest.raises(HTTPException):
                await compare_methods(params, methods=["standard_mc"], user=mock_user)

    async def test_scrapers_router_exception(self):
        from src.routers.scrapers import trigger_scraper
        mock_user = {"id": "test-user"}
        with patch("src.routers.scrapers.publish_scrape_task", side_effect=Exception("Scraper failed")):
    ...
    async def test_nse_scraper_value_error(self, mock_pw):
    ...
    def test_methods_base_protocol(self):
        ...
        class Dummy:
            def price(self, params): pass
        assert isinstance(Dummy(), NumericalMethod)

    def test_analytical_asian(self):
        method = BlackScholesAnalytical()
        params = OptionParams(
            underlying_price=100, strike_price=100, maturity_years=1,
            volatility=0.2, risk_free_rate=0.05, option_type="call"
