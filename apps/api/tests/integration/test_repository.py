...
async def test_price_result_flow(sample_option_params, cleanup_ids) -> None:
    ...
    res = await repository.upsert_price_result(
        option_id,
    ...
    assert any(
        e["id"] == res["id"] for e in await repository.get_experiments_by_method("analytical")
    )
...
async def test_notification_workflow(cleanup_ids, test_user_id) -> None:
    ...
    assert any(
        n["id"] == notif_id for n in await repository.get_notifications(user_id, unread_only=True)
    )
    ...
    # Unread only = False
    assert any(
        n["id"] == notif_id for n in await repository.get_notifications(user_id, unread_only=False)
    )
...
async def test_market_data_operations(sample_option_params, cleanup_ids) -> None:
    ...
    await repository.insert_market_data(
        [{"option_id": option_id, "trade_date": today.isoformat(), "data_source": "spy"}]
    )
    cleanup_ids["market_data"].append(option_id)
    assert any(
        d["option_id"] == option_id
        for d in await repository.get_market_data("spy", trade_date=today)
    )
...
async def test_filters(sample_option_params, cleanup_ids) -> None:
    ...
    today = datetime.date.today()
    await repository.insert_market_data(
        [{"option_id": option_id, "trade_date": today.isoformat(), "data_source": "nse"}]
    )
    cleanup_ids["market_data"].append(option_id)
...
async def test_redis_publish_user(sample_option_params, cleanup_ids, test_user_id) -> None:
    option_id = await repository.upsert_option_parameters(sample_option_params)
    cleanup_ids["option_parameters"].append(option_id)
    await repository.upsert_price_result(
        option_id,
