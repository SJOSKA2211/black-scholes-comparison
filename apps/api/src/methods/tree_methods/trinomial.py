...
class TrinomialTree:
    ...
    def price(self, params: OptionParams) -> PriceResult:
        ...
        def _solve(p: OptionParams) -> tuple[float, float, float]:
            ...
            p_u = (num_up / den) ** 2
            p_d = ((np.exp(p.volatility * np.sqrt(dt / 2)) - np.exp(p.risk_free_rate * dt / 2)) / den) ** 2
            p_m = 1.0 - p_u - p_d
            ...
            for step in range(self.num_steps - 1, -1, -1):
                ...
                if p.is_american:
                    ...
                    exercise = np.maximum(s_step - p.strike_price, 0) if p.option_type == "call" else np.maximum(p.strike_price - s_step, 0)
        ...
        vega = (get_p(params.model_copy(update={"volatility": params.volatility + h_v})) - computed_price) / h_v
        theta = (
            -(computed_price - get_p(params.model_copy(update={"maturity_years": max(0.0001, params.maturity_years - h_t)}))) / h_t
            if params.maturity_years > h_t else 0.0
        )
        rho = (get_p(params.model_copy(update={"risk_free_rate": params.risk_free_rate + h_r})) - computed_price) / h_r
