...
class OptionRepository:
    ...
    async def upsert_option_params(self, params_dict: dict[str, Any]) -> str:
        ...
        try:
            ...
            for key, val in params_dict.items():
                if key in ["underlying_price", "strike_price", "maturity_years", "volatility", "risk_free_rate", "option_type", "is_american", "market_source"]:
            ...
            SUPABASE_QUERY_DURATION.labels(table="option_parameters", operation="upsert").observe(time.perf_counter() - start_time)
            if not result.data:
    ...
    async def insert_method_result(self, result_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            SUPABASE_QUERY_DURATION.labels(table="method_results", operation="insert").observe(time.perf_counter() - start_time)
    ...
    async def upsert_market_data(self, records: list[dict[str, Any]] | pd.DataFrame) -> int:
        ...
        try:
            result = self.client.table("market_data").upsert(records, on_conflict="option_id,trade_date").execute()
            SUPABASE_QUERY_DURATION.labels(table="market_data", operation="upsert_batch").observe(time.perf_counter() - start_time)
            return len(result.data)
        except Exception as e:
            # Re-raise as RepositoryError for tests to catch
            raise RepositoryError(str(e)) from e

    async def log_audit(self, run_id: str | uuid.UUID, step: str, status: str, rows: int = 0, message: str | None = None) -> None:
    ...
    async def get_scrape_runs(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        result = self.client.table("scrape_runs").select("*").order("started_at", desc=True).limit(limit).execute()
        return result.data
    ...
    async def insert_notification(self, user_id: str, title: str, body: str, severity: str = "info", channel: str = "in_app") -> list[dict[str, Any]]:
        ...
        result = self.client.table("notifications").insert({
            "user_id": user_id, "title": title, "body": body, "severity": severity, "channel": channel
        }).execute()
        return result.data

    async def get_notifications(self, user_id: str, limit: int = 50, unread_only: bool = False) -> list[dict[str, Any]]:
        """Retrieve notifications for a user."""
        query = self.client.table("notifications").select("*").eq("user_id", user_id).order("created_at", desc=True).limit(limit)
        if unread_only:
